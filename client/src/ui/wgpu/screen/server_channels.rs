        ...
        for (idx, channel) in client.server.load().as_ref().unwrap().channels.load().iter().enumerate() {
            ...
            self.container.add(Arc::new(RwLock::new(Box::new(Button {
                ...
                on_click: Arc::new(Box::new(|button, client| {
                    let channel_switch = ClientPacket::SwitchChannel { channel: button.data.clone() }.encode().unwrap();
                    client.server.load().as_ref().unwrap().connection.get().unwrap().send_reliable(&channel_switch).block_on().unwrap();
                })),
            }))));
        }
    }
